...
def load_and_clean_data(lakeCSVPath, lakeMetaDataPath):
    ...
    data_pivoted["Time"] = pd.to_datetime(
        data_pivoted["Time"], format="%H:%M:%S", errors="coerce"
    ).dt.time

    excel = pd.read_excel(lakeMetaDataPath)
    excel = excel[
        ["GEMS Station Number", "Station Identifier", "Latitude", "Longitude"]
    ]

    data_pivoted = pd.merge(
        data_pivoted,
        excel,
        left_on="Station",
